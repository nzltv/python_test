...
def main():
    ...
    coursesBtn.click()
    # выбираем валюту 
    cur_select = Select(driver.find_element_by_id('ctl00_PageContent_CurrencySelect'))
    cur_select.select_by_visible_text('USD/RUB - Доллар США к российскому рублю')
    # выбираем даты
    ...
    cur_select = Select(driver.find_element_by_id('ctl00_PageContent_CurrencySelect')) 
    cur_select.select_by_visible_text('EUR/RUB - Евро к российскому рублю')
    selectDates(driver, firstDay, lastDay, currentMonth, currentYear)
    # если записи есть - формируем таблицу
    if check_exists_by_xpath(driver, "//b[contains(text(), 'Записей не найдено')]"):
...
def selectDates(driver, firstDay, lastDay, currentMonth, currentYear):
    ...
    Select(driver.find_element_by_id('d1day')).select_by_visible_text(firstDay)
    Select(driver.find_element_by_id('d1month')).select_by_visible_text(currentMonth)
    Select(driver.find_element_by_id('d1year')).select_by_visible_text(currentYear)
    #выбираем последнее число месяца
    Select(driver.find_element_by_id('d2day')).select_by_visible_text(lastDay)
    Select(driver.find_element_by_id('d2month')).select_by_visible_text(currentMonth)
    Select(driver.find_element_by_id('d2year')).select_by_visible_text(currentYear)
    time.sleep(3)
...
def getTable(driver, xpath, currency):
    ...
    for r in range(2, len(tbl.find_elements(By.TAG_NAME, "tr"))):
        ...
        if row.find_elements(By.TAG_NAME, "td")[1].text.replace(',','.') != '-':
            ...
            dict[headres[1]] = 0
            dict[headres[2]] = 0
        if row.find_elements(By.TAG_NAME, "td")[3].text.replace(',','.') != '-':
